...
if __name__ == '__main__':
    ...
    server_model = AutoModel.from_pretrained(model_name,
                                                      quantization_config=bnb_config,
                                                      trust_remote_code=True,
                                                      use_auth_token=True,
                                                    )    
    
    wandb.init(project="TriplePlay Final  Inferences", config=vars(args),name="with Quantization")
        ...
        with torch.no_grad():
            ...
            for client_idx in range(client_num):
                ...
                logrecord += ' Test site-{:d}| Test Acc: {:.4f}'.format(
                    ...
                    client_idx, test_acc)
                wandb.log({f"Test Acc{client_idx}":test_acc})
